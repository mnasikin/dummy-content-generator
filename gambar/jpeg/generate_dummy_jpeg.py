#!/usr/bin/env python3
"""
generate_dummy_jpeg.py

Generate dummy file JPEG berukuran presisi (byte-exact) buat testing
...
dan keterangan "Dummy JPEG".

JPEG itu format biner terkompresi, jadi bisa di-padding
...
python3 generate_dummy_jpeg.py --prefix Contohnya-JPEG
"""
...
def make_abstract_background(width: int, height: int, label: str = "") -> bytes:
    ...
    if PILLOW_AVAILABLE:
        ...
        draw.text((60, height // 2 - 30), JPEG_TITLE, fill="#1a1a1a", font=font_large)
        draw.text((60, height // 2 + 10), f"Ukuran: {format_label(label)}", fill="#333333", font=font_small)
        draw.text((60, height // 2 + 40), JPEG_CAPTION, fill="#555555", font=font_small)
...
def pad_jpeg_to_size(jpeg_bytes: bytes, target_bytes: int) -> bytes:
    base_bytes = len(jpeg_bytes)
    if base_bytes > target_bytes:
        raise ValueError(
            f"Base JPEG ({base_bytes} bytes) sudah lebih besar dari target ({target_bytes} bytes). "
            "Kurangi jumlah shape background atau naikkan target size."
        )

    padding_len = target_bytes - base_bytes
    padding = b'\x00' * padding_len
    padded = jpeg_bytes + padding
...
def main():
    parser = argparse.ArgumentParser(description="Generate dummy JPEG ukuran presisi.")
    parser.add_argument(
        "--sizes",
        type=str,
        default=None,
    ...
    parser.add_argument(
        "--prefix", type=str, default="Contohnya-JPEG", help="Prefix nama file output."
    )
    parser.add_argument(
        "--binary",
        action="store_true",
    ...
    for label, size in sorted(targets.items(), key=lambda kv: kv[1]):
        jpeg_bytes = make_abstract_background(WIDTH, HEIGHT, format_label(label))
        base_bytes = len(jpeg_bytes)
        if base_bytes > size:
            print(f"{label:>8} -> SKIP, base JPEG ({base_bytes} bytes) > target ({size} bytes)")
            continue
        padded = pad_jpeg_to_size(jpeg_bytes, size)
        out_path = os.path.join(args.outdir, f"{args.prefix}-{format_label(label)}.jpeg")
