#!/usr/bin/env python3
"""
generate_dummy_heif.py

Generate dummy file HEIF berukuran presisi (byte-exact-ish target with filler)
buat testing upload/download/parsing gambar. Tiap file berisi gambar HEIF asli
(bukan cuma file acak ber-ekstensi .heif), lalu dipad dengan trailing filler
...
Catatan:
- Menggunakan pillow-heif agar output benar-benar disimpan sebagai HEIF/HEIC via Pillow plugin.
- HEIF bisa disimpan dari Pillow setelah plugin didaftarkan dengan register_heif_opener().
- pillow-heif juga mendukung penyimpanan ke format "HEIF" lewat parameter format saat save.
- File dibikin aman dan statis untuk kebutuhan dummy testing.
...
python3 generate_dummy_heif.py --prefix Contohnya-HEIF
"""
...
def make_image(label: str, target_size: int) -> Image.Image:
    ...
    title = fit_text(draw, HEIF_TITLE, font_big, width - 80)
    sub1 = fit_text(draw, format_label(label), font_small, width - 80)
    sub2 = fit_text(draw, HEIF_CAPTION, font_small, width - 80)
...
def build_heif(label: str, target_size: int) -> bytes:
...
def pad_heif_to_size(heif_bytes: bytes, target_bytes: int) -> bytes:
    base_len = len(heif_bytes)
    if base_len > target_bytes:
        raise ValueError(
            f"Base HEIF ({base_len} bytes) sudah lebih besar dari target ({target_bytes} bytes)."
        )
    if base_len == target_bytes:
        return heif_bytes
    filler = b"\x00" * (target_bytes - base_len)
    return heif_bytes + filler


def main():
    parser = argparse.ArgumentParser(description="Generate dummy HEIF ukuran presisi.")
    parser.add_argument(
        "--sizes",
        type=str,
        default=None,
        help="Comma-separated ukuran, misal '25kb,1mb,15mb'. Default: varian ukuran umum HEIF sampai 15MB.",
    )
    parser.add_argument("--outdir", type=str, default=".", help="Folder output.")
    parser.add_argument(
        "--prefix", type=str, default="Contohnya-HEIF", help="Prefix nama file output."
    )
    parser.add_argument(
        "--binary",
        action="store_true",
    ...
    for label, size in sorted(targets.items(), key=lambda kv: kv[1]):
        heif_base = build_heif(format_label(label), size)
        base_bytes = len(heif_base)
        if base_bytes > size:
            print(f"{label:>8} -> SKIP, base HEIF ({base_bytes} bytes) > target ({size} bytes)")
            continue

        padded = pad_heif_to_size(heif_base, size)
        out_path = os.path.join(args.outdir, f"{args.prefix}-{format_label(label)}.heif")
