...
Script Python buat generate dummy PNG berukuran presisi (byte-exact) untuk keperluan testing upload/download/parsing gambar.

File yang dihasilkan tetap PNG valid (bisa dibuka di browser atau PNG viewer mana pun, bukan cuma file random bytes yang dipaksa berekstensi `.png`), dan isinya beneran multi-element (bukan cuma 1 element + padding tersembunyi) — jumlah element menyesuaikan besar target ukuran.

PNG itu format container terkompresi, jadi bisa di-padding byte-exact lewat estimasi + koreksi iteratif.
...
python3 generate_dummy_png.py

# generate ukuran tertentu aja
python3 generate_dummy_png.py --sizes 50kb,25mb

# ukuran custom yang gak ada di default
python3 generate_dummy_png.py --sizes 250kb,2.5mb,777kb

# ganti folder output
python3 generate_dummy_png.py --outdir ./png

# ganti prefix nama file (default: Contohnya-PNG)
python3 generate_dummy_png.py --prefix MyFile

# pakai definisi binary (1 MB = 1024x1024 bytes / MiB) instead of decimal (default)
python3 generate_dummy_png.py --sizes 10mb --binary
```
...
Nama file output ikut pattern `{prefix}-{ukuran}.png`, misal `Contohnya-PNG-50KB.png`, `Contohnya-PNG-25MB.png`.
...
### Kustomisasi teks di dalam PNG

Edit langsung dua variabel ini di bagian atas file `generate_dummy_png.py`:
...
3. PNG itu format container terkompresi, jadi padding-nya gak ikut dikompres dan ukuran akhir tetap presisi byte-exact.
